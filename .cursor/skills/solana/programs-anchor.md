...
## Compatibility Notes for Anchor 0.32.1

To resolve build conflicts with certain crates in Anchor 0.32.1, run these cargo update commands in your project root:
...
For `solana-program` dependency conflicts, use `anchor_lang::solana_program` (exported by Anchor) instead of adding a direct dependency. If conflicts persist, resolve them via a `[patch.crates-io]` section in `Cargo.toml` or by setting `solana_version` in `Anchor.toml` to align Solana versions across dependencies.
