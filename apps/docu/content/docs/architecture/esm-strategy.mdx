...
`tsgo` is the **TypeScript Go compiler**. We use it because it’s fast and works well as the default tool for monorepo **compilation** and `--noEmit` **typechecks**.
...
**ESM requires explicit extensions** at runtime, but TypeScript can resolve extensionless imports in source.
...
**Best Practice:**
- Omit extensions in TypeScript when the runtime/tooling supports it
- Use `.js` extensions when targeting strict ESM environments
