...
This codebase uses different schema libraries depending on the runtime context:
- **Zod**: Recommended for Next.js API routes (e.g., exported POST handlers in `app/api/` routes)
- **TypeBox**: Used for Fastify route schemas and validation (all Fastify route handlers)

**Zod for Next.js API Routes:**
- Validate at boundaries: Next.js API routes, forms, external APIs
...
**TypeBox for Fastify Routes:**
- Use TypeBox for all Fastify route schemas (request/response validation)
- Native JSON Schema - no conversion overhead
- Automatic OpenAPI generation
- Type-safe with TypeBox type provider

**Example (Next.js API Route with Zod):**
```typescript
// ✅ Good: Zod schema with type inference for Next.js API routes
import { z } from 'zod'
...
// ✅ Good: Validate at Next.js API boundary
...
```typescript
// ✅ Good: TypeBox schema for Fastify route handlers
...
// ✅ Good: TypeBox schema in Fastify route
fastify.post('/users', {
  schema: {
    body: UserSchema
  }
}, async (request) => {
  const user = request.body // Type-safe after validation
  // ...
})
```
