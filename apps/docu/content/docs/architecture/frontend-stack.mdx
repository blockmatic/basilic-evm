...
- **@tanstack/react-query**: Primary solution for ANY async operation that benefits from caching/state management
  - **Versatile**: Accepts ANY async `queryFn`—not just HTTP calls. Can be local computations, localStorage reads, file operations, or any Promise-returning function
  - Use `@lukemorales/query-key-factory` for centralized query keys
  - All TanStack features work identically: caching, deduping, background refetching, stale-while-revalidate, devtools
...
- **ahooks**: Utility hooks for state management
  - `useSetState`: Grouped state management (game engine, form state, ephemeral UI state)
  - `useLocalStorageState`: localStorage state management
...
### TanStack Query Versatility

TanStack Query accepts ANY async `queryFn`—not just HTTP calls. Use it for any async operation that benefits from caching or state management:
...
const { data } = useQuery({
  queryKey: ['local-state'],
  queryFn: () => Promise.resolve(expensiveLocalCalculation()),
})

// Server data fetching (traditional use case)
const { data } = useQuery({
...
**Key Benefits**: All TanStack features work identically regardless of data source: caching by `queryKey`, deduping, background refetching, stale-while-revalidate, and devtools.
...
- Use TanStack Query for ANY async operation that benefits from caching/state management (not just HTTP calls)
...
- Use `ahooks.useSetState` for grouped state not in URL (game engine, form state)
- Use `ahooks.useLocalStorageState` for localStorage state management
- For one-off async operations without caching needs, use plain async/await in event handlers
