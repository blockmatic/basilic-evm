...
The API works with any PostgreSQL-compatible database via `DATABASE_URL`. Common options include:

- ✅ **Supabase** - Managed Postgres with branching and local development CLI (optional)
- ✅ **Cloud SQL / RDS** - Enterprise-grade managed PostgreSQL
- ✅ **Self-hosted PostgreSQL** - Full control over infrastructure
- ✅ **PGLite** - In-memory PostgreSQL for testing and preview environments (`PGLITE=true`)

**Migration**: Changing databases only requires updating `DATABASE_URL` - no code changes needed.
...
Migrations are managed with `drizzle-kit` and follow a host-agnostic strategy:

**Migration Generation:**
- `db:generate` - Generate migration files from schema changes

**Migration Execution:**
- **PostgreSQL**: Migrations run at **build time** via `pnpm build` → `pnpm db:migrate` → `tsc`
  ...
  - Works with any deployment platform (Docker, Railway, Fly.io, Render, etc.)
- **PGLite**: Migrations run at **runtime** when instance is created
  - PGLite instance doesn't exist at build time
  - Used for preview branches and local development

**Environment Configuration:**
- `PGLITE=true` - Uses in-memory PGLite database (preview branches, local dev)
- `PGLITE=false` (default) - Uses PostgreSQL via `DATABASE_URL`

See [ADR 008: Database Platform & Strategy](/docs/adrs/008-database#migration-strategy) for detailed migration flow and architecture diagram.
