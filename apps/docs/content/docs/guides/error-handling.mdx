...
- [Logging Guide](/docs/guides/logging) - Logging patterns with `@repo/utils/logger`. All logging in `@repo/error` uses `@repo/utils/logger` for unified logging across the monorepo.
- [Security Guide](/docs/guides/security) - Security best practices and PII handling. Sentry's built-in PII scrubbing automatically handles sensitive data.
