...
### 6. Error Logging with captureError

When using `captureError` from `@repo/error` in Fastify, pass Fastify's native logger to ensure errors are logged with request context and avoid duplicate logging:
...
fastify.setErrorHandler((error: FastifyError, request, reply) => {
  // Pass request.log to use Fastify's native logger
...
**Benefits:**
- Errors logged through Fastify's native Pino logger
- Automatic request context (requestId via `requestIdLogLabel: 'reqId'`)
- No duplicate logging (captureError handles all error logging internally)
- Consistent with Fastify's logging strategy

If `logger` is omitted, `captureError` defaults to the internal logger from `@repo/utils/logger`.
